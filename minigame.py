...
def search_targets() -> list:
    ...
    _, bar_bw = cv2.threshold(bar_gray, 70, 255, cv2.THRESH_BINARY)
    ...
    for target in contours:
        if cv2.contourArea(target) >= 100:
            ...
            # Value might change based on difficult shovel?
            offset = 4
            targets.append([cx-offset, cx+offset])
